...
def get_inventory_ids():
    cursor.execute("SELECT inventory_id FROM warehouse_inventory ORDER BY inventory_id")
    return [row[0] for row in cursor.fetchall()]
...
def create_order(customer_id):
    ...
    try:
        cursor.callproc("create_online_order", (customer_id, new_order_id))
...
def create_order_details(order_id):
    ...
    for _ in range(random.randint(1,10)):
        ...
        try:
            cursor.callproc("create_online_order_detail", (order_id, inventory_id, buy_quantity))
